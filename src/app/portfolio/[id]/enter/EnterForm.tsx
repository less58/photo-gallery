...
export default function EnterForm({ portfolioId, daysRemaining }: { portfolioId: string; daysRemaining: number | null }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch(`/api/portfolio/${portfolioId}/enter`, {
        method: 'POST',
      ...
      if (res.ok) {
        window.location.replace(`/portfolio/${portfolioId}`)
      } else {
  ...
  const expiryColor = daysRemaining !== null && daysRemaining <= 3
    ? '#f87171'
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-8 flex flex-col items-center gap-6">
          ...
          {daysRemaining !== null && daysRemaining > 0 && (
            <div
              ...
              style={{ background: 'rgba(255,255,255,0.06)', border: `1px solid ${expiryColor}30` }}
            >
              <Clock size={14} style={{ color: expiryColor, flexShrink: 0 }} />
              <p className="text-sm font-semibold text-center" style={{ color: expiryColor }}>
                {daysRemaining === 1
                  ? 'נותר יום אחד בלבד לבחירת התמונות!'
                  : `הגלריה פתוחה עוד ${daysRemaining} ימים`}
              </p>
            </div>
          )}

          {daysRemaining !== null && daysRemaining <= 0 && (
            <div className="w-full rounded-xl px-4 py-3 text-center" style={{ background: '#ef444415', border: '1px solid #ef444430' }}>
              <p className="text-sm font-semibold text-red-400">תקופת הבחירה הסתיימה</p>
            </div>
          )}
